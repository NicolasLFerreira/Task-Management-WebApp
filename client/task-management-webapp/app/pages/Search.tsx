...
const Search = () => {
	...
	const fetchAllLists = async (): Promise<ListDto[]> => {
		try {
			...
			for (const board of boards) {
				try {
					const listsResponse =
						await ListService.getApiListsBoardByBoardId({
					...
					console.warn(
						`Failed to fetch lists for board ${board.id}:`,
						err
					);
				}
			}
	...
	const performSearch = async (
		...
		try {
			if (searchTab === "tasks") {
				...
				const filteredLists = allLists.filter((list: ListDto) =>
					list.title?.toLowerCase().includes(term.toLowerCase())
				);
	...
	const handleSearch = async (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === "Enter") {
			...
			if (term) {
				...
				navigate(
					`/search?q=${encodeURIComponent(term)}&tab=${activeTab}`
				);
			} else {
				fetchAllData();
			}
	...
	const fetchAllData = async () => {
		setLoading(true);
		try {
			const [tasksResponse, boardsResponse, usersResponse] =
				await Promise.all([
	...
	const tabs = [
		{
			id: "tasks" as SearchTab,
			label: "Tasks",
			icon: CheckSquare,
			count: results.tasks.length,
		},
		{
			id: "boards" as SearchTab,
			label: "Boards",
			icon: Layout,
			count: results.boards.length,
		},
		{
			id: "lists" as SearchTab,
			label: "Lists",
			icon: Folder,
			...
			{results.tasks.map((task) => (
				<div key={task.id} className="relative group">
					<div onClick={() => handleTaskClick(task.id!)}>
						<TaskCard
							task={task}
							onViewClick={() => handleTaskClick(task.id!)}
						/>
					</div>
